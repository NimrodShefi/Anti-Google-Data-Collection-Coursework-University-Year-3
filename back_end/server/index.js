...
app.post('/user_details', (request, response) => {
    ...
    user.then(data => {
        var userDetails = {
            ...
            familyName: encryptData(data.lastName),
    ...
    user.then(userData => {
        if (userData.maxRequests > userData.currentRequests) {
            websitesList.then(websites => {
                /* this is put here, for when the option to add websites to the
                database is added to the application, and DB may be updated while users use the app */
                getDbMax(websites); 
...
/* create timer to delete all users from the database at the end of a day. This
is done to ensure that a user can't logout and relogin and restart his daily
subscription. He has to wait until the next day */
